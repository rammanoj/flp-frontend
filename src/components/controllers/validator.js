...
const handleAllFields = (
  ...
  if (name === "remember_me") {
    ...
    if (usingDisabled) {
      formFieldValues[formFields.indexOf(name)] = value;
      let checkEmpty = formFieldValues.some(elem => elem === "");
      let error = false;
      for (let i in formFields) {
        if (current_validator.hasOwnProperty(formFields[i])) {
          if (formFields[i] === "state") {
            continue;
          }
          if (current_validator[formFields[i]].errors.length !== 0) {
            error = true;
          }
        }
      }
